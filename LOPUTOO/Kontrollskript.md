...
$TargetServerIP = "192.168.124.64" # Linux serveri IP
...
Add-Task "2. Ketta F: initsialiseerimine" 1 {
...
# --- 4. HINDE ARVUTAMINE ---
$HinneString = switch ($TotalPoints) {
    {$_ -ge 22} { "5 (Väga hea)" }
    {$_ -ge 17} { "4 (Hea)" }
    {$_ -ge 12} { "3 (Rahuldav)" }
...
$FullApiUrl = "http://$($TargetServerIP):5000/api/upload"
Write-Host "Saadan andmeid serverisse: $FullApiUrl" -ForegroundColor Yellow

try {
    Invoke-RestMethod -Uri $FullApiUrl `
                      -Method Post `
                      -Body $Payload `
    ...
    Write-Host "EDUKAS! Punktid: $TotalPoints / 25 | Hinne: $HinneString" -ForegroundColor Green
} catch {
