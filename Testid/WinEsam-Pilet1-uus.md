...
# DC2 IP KONTROLL (Staatiline vahemik < 100)
...
if ($DC2DNS) {
    ...
    if ($lastOctet -lt 100) {
        $dc2IpDet = "Leitud IP: <b>$ip</b> <span style='color:green'>(OK: Tundub staatiline, lõpp < 100)</span>"
        $dc2IpStatus = $true
    } else {
        $dc2IpDet = "Leitud IP: <b>$ip</b> <span style='color:orange'>(HOIATUS: Lõpeb üle 100, võib olla dünaamiline DHCP IP!)</span>"
        $dc2IpStatus = $true # Anname punktid, aga kuvame hoiatuse
    }
}
Add-Result "DC2 staatiline IP" "DNS kirje viitab DC2 staatilisele IP-le (< .100)" $dc2IpDet $dc2IpStatus 0.5
...
if ($DHCPScope) {
    ...
    $dhcpDetails += "<li>Skoop: <b style='color:green'>Jah (+0.25p)</b> [Nimi: <b>$scopeName</b>, ID: $($DHCPScope.ScopeId)]</li>"
    ...
    # Ruuteri (Gateway) Info (Visuaalseks infoks)
    $RouterOption = Get-DhcpServerv4OptionValue -ScopeId $DHCPScope.ScopeId -OptionId 3 -ErrorAction SilentlyContinue
    if ($RouterOption) {
        $dhcpDetails += "<li>Ruuter (Option 3): <b>$($RouterOption.Value -join ", ")</b></li>"
    } else {
        $dhcpDetails += "<li>Ruuter (Option 3): <b style='color:orange'>Puudub</b></li>"
    }
    ...
    if ($DNSOptions -and $DNSOptions.Value.Count -ge 2) {
        $dhcpTasksMet++; $dhcpDetails += "<li>DNS serverid: <b style='color:green'>Täidetud (+0.25p)</b> [IP-d: <b>$($DNSOptions.Value -join ", ")</b>]</li>"
    } else { 
        $dnsFound = if ($DNSOptions) { $DNSOptions.Value -join ", " } else { "Puudu" }
        $dhcpDetails += "<li>DNS serverid: <b style='color:red'>Vähem kui 2 seadistatud</b> [Leitud: $dnsFound]</li>" 
    }

    # Reservatsioonide detailsus
    ...
    if ($Reservations.Count -gt 0) {
        $dhcpTasksMet++
        $resList = "<ul style='margin-top:5px; margin-bottom:5px; padding-left:15px;'>"
        foreach ($res in $Reservations) {
            $mac = $res.ClientId -replace '-',':'
            $rName = if ($res.Name) { $res.Name } else { "Nimetu" }
            $resList += "<li>IP: <b>$($res.IPAddress)</b> | MAC: $mac | Nimi: $rName</li>"
        }
        $resList += "</ul>"
        $dhcpDetails += "<li>Reservatsioonid: <b style='color:green'>Täidetud (+0.25p)</b> $resList</li>"
    } else { 
...
# 7.3 Sisuhaldussüsteemi paigaldus
