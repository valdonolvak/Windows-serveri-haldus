<img width="1075" height="756" alt="image" src="https://github.com/user-attachments/assets/dd45bd63-0815-43a1-a398-c9879048cfb8" />### Pilet 4 ###
...
if ($DHCPScope) {
    ...
    } else { $dhcpDetails += "<li>Rendiaeg 4h: <b style='color:red'>Vale ($($DHCPScope.LeaseDuration))</b></li>" }
    ...
    if ($Reservations.Count -gt 0) {
        $dhcpTasksMet++; $resInfo = ($Reservations | ForEach-Object { if($_.Name){$_.Name}else{$_.IPAddress} }) -join ", "
    ...
    } else { $dhcpDetails += "<li>DNS serverid (min 2): <b style='color:red'>Puudu / Vähem kui 2</b></li>" }
} else {
    $dhcpDetails += "<li>Skoop, Rendiaeg, Reservatsioonid, DNS: <b style='color:red'>Kõik puudu</b></li>"
}
$dhcpDetails += "</ul>"
...
Add-Result "Seadista DHCP teenusele failover" "Partneriks on DC2" (if ($foStatus) { "Partner DC2 leitud" } else { "Failover puudu/vale" }) $foStatus 1.0
...
if ($Haldur) {
    if ($Haldur.DistinguishedName -match "OU=Kasutajad") { $haldurTasksMet++; $haldurDetails += "<li>Asub OU-s Kasutajad: <b style='color:green'>Jah (+0.25p)</b></li>" }
    if ($Haldur.MemberOf -match "Domain Admins|Domeeni administraatorid") { $haldurTasksMet++; $haldurDetails += "<li>Domain Admins grupis: <b style='color:green'>Jah (+0.25p)</b></li>" }
} else { $haldurDetails += "<li>Kasutaja 'haldur': <b style='color:red'>Ei leitud</b></li>" }
$haldurDetails += "</ul>"
...
if ($AllComps) {
    if ($AllComps | Where-Object { $_.DistinguishedName -match "OU=Arvutid" }) {
        $compTasksMet = 2; $compDetails += "<li>Domeenis leitud: <b style='color:green'>Jah (+0.25p)</b></li><li>Asub OU-s Arvutid: <b style='color:green'>Jah (+0.25p)</b></li>"
    } else {
        $compTasksMet = 1; $compDetails += "<li>Domeenis leitud: <b style='color:green'>Jah (+0.25p)</b></li><li>Asub OU-s Arvutid: <b style='color:orange'>Ei, asub mujal</b></li>"
    }
} else { $compDetails += "<li>Klientmasin: <b style='color:red'>Ei leitud</b></li>" }
$compDetails += "</ul>"
...
Add-Result "DNS A-kirjed kõikide Linux serverite jaoks" "Vähemalt 1 manuaalne A-kirje" "$($DnsRecords.Count) tk leitud" ($DnsRecords.Count -gt 0) 1.0
...
foreach ($Dept in $TestUsers.Keys) {
    $FoundUser = Get-ADUser -Filter "Name -eq '$($TestUsers[$Dept])'" -Properties DistinguishedName -ErrorAction SilentlyContinue
    if ($FoundUser -and $FoundUser.DistinguishedName -match "OU=$Dept") { $UsersFoundCount++ }
}
Add-Result "Loodud skript impordib AD kasutajad (kasutajad.csv)" "Kasutajad leitud õigetes OU-des" "$UsersFoundCount / 7 testkasutajat leitud" ($UsersFoundCount -gt 0) 1.0
