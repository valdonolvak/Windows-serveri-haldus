...
.SYNOPSIS
    ...
    Sisaldab täpsustatud pealkirju, DC2 IP lollikindlamat tuvastust ja on taastatud pikas/loetavas formaadis.
...
# Parem DC2 tuvastus otse AD-st, kaasates ka IP-aadressi info
...
# 7.5 GPO Kogukond (PAINDLIK KETTATÄHE OTSIJA VEO PARANDUS)
...
if ($GpoK) {
    ...
    if ($xmlString -match 'letter="([A-Za-z]):"' -or $xmlString -match 'Drive Letter="([A-Za-z]):"') {
        ...
        if ($foundLetter -eq 'Y') {
            ...
            # Siin on Powershelli muutuja kuvamise viga parandatud! -> ${foundLetter}
...
# 7.10 GPO Isiklik (PAINDLIK KETTATÄHE OTSIJA VEO PARANDUS)
