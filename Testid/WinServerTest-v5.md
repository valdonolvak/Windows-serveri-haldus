...
if ($DHCPScope) {
    ...
    if ($Reservations -and $Reservations.Count -gt 0) {
        $dhcpTasksMet++
        $dhcpDetails += "<li>Reservatsioonid: <b style='color:green'>Täidetud (+0.25p)</b></li>"
    } else {
    ...
    $dhcpDetails += "<li>DNS serverid (min 2): <b style='color:red'>Skoop puudub</b></li>"
}

$Failover = Get-DhcpServerv4Failover -ErrorAction SilentlyContinue
if ($Failover -and (($Failover.PartnerServer -join " ") -match "DC2")) {
    $dhcpTasksMet++
    $dhcpDetails += "<li>Failover partner DC2: <b style='color:green'>Täidetud (+0.25p)</b></li>"
} else {
...
Add-Result "Kasutaja haldur (OU + Grupp)" "OU=Kasutajad ja Domain Admins" $haldurDetails $haldurStatus 0.5 $haldurScore
...
Add-Result "Windows 11 klientmasin" "Domeenis ja OU-s Arvutid" $compDetails $compStatus 0.5 $compScore
