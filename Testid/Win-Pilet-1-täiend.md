...
function Check-GPOSettings {
    ...
    # Loeme lingid turvalisemalt (nii objektide kui ka massiividena)
    $linkText = if ($Xml.GPO.LinksTo) {
        $links = $Xml.GPO.LinksTo
        if ($links -is [array]) {
            $links | ForEach-Object { "$($_.SOMName) $($_.SOMPath)" } -join " "
        } else {
            "$($links.SOMName) $($links.SOMPath)"
        }
    } else { "" }
...
if ($Haldur) {
    ...
    # Kindlaim viis kontrollida grupi kuuluvust (kaasa arvatud Primary Group)
...
# OU=Personal otsing asendatud lihtsalt sõnaga "Personal", sest kanooniline asukoht on domain.local/Kasutajad/Personal
...
# Kasutame Get-Website tagamaks, et saame füüsilise asukoha kätte korrektsemalt
...
if ($IISSite) {
    # Loeme füüsilise asukoha otse IIS-i seadetest ja laiendame keskkonnamuutujaid (nagu %SystemDrive%)
    ...
    if ($physPath -and (Test-Path $physPath)) {
        ...
        $cmsDet += "<li>Füüsiline kaust: <b style='color:red'>Ei eksisteeri või ei leitud ($physPath)</b></li>"
    }
} else {
